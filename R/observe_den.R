...
#'
#' This will return the denominator of the test statistic for data.
#'
...
observe_den <- function(sim, omega = 1){
  ...
  var_center <- 1/(sum(group_sizes)-g)*sum(deviations_center)
  var_radius <- 1/(sum(group_sizes)-g)*sum(deviations_radius)
